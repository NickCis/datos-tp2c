...
void usage_2( void ) {
	...
	printf("\ti <k>  -- Insertar <k> (entero) tanto como llave y valor.\n");
	printf("\tf <k>  -- Encontrar el valor de la llave <k>.\n");
...
void find_and_print(node * root, int key, bool verbose) {
	...
	if (r == NULL)
		printf("Registro no encontrado para la llave %d.\n", key);
	else 
		printf("Registro en %lx -- llave %d, valor %d.\n",
...
void find_and_print_range( node * root, int key_start, int key_end,
	...
	if (!num_found)
		...
		for (i = 0; i < num_found; i++)
			printf("Llave: %d   Ubicación: %lx  Valor: %d\n",
					returned_keys[i],
...
node * insert_into_leaf_after_splitting(node * root, node * leaf, int key, record * pointer) {
	...
	if (temp_keys == NULL) {
		perror("Array temporal de llaves.");
		exit(EXIT_FAILURE);
	}

	temp_pointers = malloc( order * sizeof(void *) );
	if (temp_pointers == NULL) {
		perror("Array temporal de punteros.");
		exit(EXIT_FAILURE);
	}
...
node * insert_into_node_after_splitting(node * root, node * old_node, int left_index, 
	...
	if (temp_pointers == NULL) {
		perror("Array temporal de punteros para división de nodos.");
		exit(EXIT_FAILURE);
	}
	temp_keys = malloc( order * sizeof(int) );
	if (temp_keys == NULL) {
		perror("Array temporal de llaves para división de nodos.");
		exit(EXIT_FAILURE);
	}
